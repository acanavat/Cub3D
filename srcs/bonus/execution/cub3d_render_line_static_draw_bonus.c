...
int	pick_good_wall_color(t_data *data, t_line line_cast, int tex_y)
{
	...
	if (data->map.tab_map[map_x][map_y] == MAP_DOOR_OPEN)
		return (get_image_color(data->door_open, mlx_instance, tex_x, tex_y));
	if (data->map.tab_map[map_x][map_y] == MAP_DOOR_CLOSED)
		return (get_image_color(data->door_closed, mlx_instance, tex_x, tex_y));
