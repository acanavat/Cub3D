...
/*   Updated: 2024/04/11 18:34:47 by acanavat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_map_3(char **str, char **map, int x, int y)
{
	while (map[x + 1])
	{
		while (map[x][y])
		{
			if (check(map[x][y]) == 0 && (map[x][y + 1] == 'x' || \
...
int	mob_pos(t_data *data, char str, int x, int y)
{
	if (str == 'M')
	{
		data->sprite = new_sprite(
				x - data->map.first_line + 0.5, y + 0.5, -15, 14);
		return (1);
	}
	return (0);
}
...
void	pos_player(t_data *data, char **str, int x, int y)
{
	...
	while (x < data->map.last_line)
	{
		while (str[x][++y])
		{
			...
			else if (str[x][y] == 'M' && mob_pos(data, str[x][y], x, y) == 1)
				mob++;
		}
		y = -1;
		x++;
	...
	if (player != 1 || mob > 1)
		free_exit(str, NULL, "invalid player or mob");
}
